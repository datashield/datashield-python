...
def test_logins():
    builder = (
        ...
        .add(
            "server2", "https://demo.datashield.org", token="1234abcd", profile="omics"
        )
    )
...
def test_login_validations():
    ...
    try:
        DSLoginBuilder().add(
            "server1", "https://opal-demo.obba.org", "dsuser", "P@ssw0rd"
        ).add("server1", "https://demo.datashield.org", token="1234abcd")
        raise AssertionError()
    except ValueError:
        pass
    ...
    try:
        DSLoginBuilder().add("server1", "https://opal-demo.obba.org").add(
            "server2", "https://demo.datashield.org"
        )
        raise AssertionError()
    except ValueError:
        pass
